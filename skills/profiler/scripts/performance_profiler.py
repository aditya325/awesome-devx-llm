...
_DEFAULT_EXCLUDED_DIRS = frozenset({".git", "node_modules", ".next", "dist", "build", "coverage", "__pycache__"})


def iter_files(root: Path, excluded_dirs: frozenset | None = None) -> Iterable[Path]:
    """Yield all files under *root*, skipping directories in *excluded_dirs*."""
...
def get_large_files(root: Path, threshold_bytes: int) -> List[Tuple[str, int]]:
    """Return files under *root* whose size meets or exceeds *threshold_bytes*, sorted largest first."""
...
def count_dependencies(root: Path) -> Dict[str, int]:
    """Count direct dependencies declared in package.json, requirements.txt, and go.mod."""
    ...
    if package_json.exists():
        try:
            ...
            print(f"Warning: could not parse {package_json}: {exc}", file=sys.stderr)
...
def bundle_indicators(root: Path) -> Dict[str, object]:
    """Scan *root* for build artefacts and estimate weighted bundle size in KB."""
    indicators = {
        "build_dirs_present": [],
        "bundle_like_files": 0,
    ...
    bundle_files = 0
    weight = 0.0
    build_dirs = [root / d for d in indicators["build_dirs_present"]]
    for build_dir in build_dirs:
        for path in iter_files(build_dir, excluded_dirs=frozenset({".git", "node_modules", "coverage", "__pycache__"})):
...
def format_size(num_bytes: int) -> str:
    """Return a human-readable string for a byte count (e.g. 1.5MB)."""
    units = ["B", "KB", "MB", "GB"]
...
def build_report(root: Path, threshold_bytes: int) -> Dict[str, object]:
    """Aggregate profiling results into a single report dict."""
...
def print_text(report: Dict[str, object]) -> None:
    """Print *report* as human-readable text to stdout."""
...
def main() -> int:
    """Entry point: parse arguments, run profiler, print report."""
    args = parse_args()
