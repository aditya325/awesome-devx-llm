...
if (afterGC.heapUsed > baseline.heapUsed * 1.1) {
  console.warn('Possible memory leak detected (>10% growth after GC)')
}
```
...
// Option A: reuse existing client with logging enabled at init time
import { db } from './client'

// Option B: create a separate instrumented instance
const drizzleDb = drizzle(pool, { logger: true })
